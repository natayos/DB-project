<template>
  <div id="allEventComponent">
        ...
        <div id="container">
            ...
            <div id="rate">
                <h3>{{zones[0].concert_name}}</h3>
                <div class="line"></div>
                <div class="detail">
                    <div>รอบการแสดง</div>
                    <div class="detail-info">{{zones[0].date}}</div>
                    <div>โซนที่นั่ง</div>     
                    <div class="detail-info">{{seatVal[1]}}</div>
                    <div>สถานะ</div>     
                    <div class="detail-info">{{status}}</div>
                    <div>ราคาบัตร</div>   
                    <div class="detail-info">{{seatVal[0]}}</div>
                    <div>จำนวนที่นั่ง</div>   
                    <div class="detail-info">-</div>
                </div>
                <div class="btn">
                    <a href="/complete-buy" @click="completeBuy()"><button class="confirm">ยืนยันการซื้อบัตร</button></a>
...
export default {
  ...
  async created() {
    ...
    this.price = urlParams.get('price').split('?');
    this.seatVal = this.price[0].split('seatVal=')
  ...
  methods: {
    async completeBuy() {
      await axios.post("http://localhost:3000/complete-buy", {
        user_id: localStorage.getItem('isLogin'),
        total_price: this.seatVal[0],
        status: 1,
      })
    },
    async loginnn() {
